...
for rev in range(num): 
    ...
    for v in range(crs):
        ...
        if chk_same==False or start==True:
            doc = docx.Document('1-3'+str(rev_nam)+'.docx')
            tbc = 0
            start=False
        else:
            doc = docx.Document(str(pln_num)+str(rev_nam)+'_審查意見表.docx')
        tb = doc.tables[tbc]
        tbc+=1

        # 綜合評分
        score = shtgf.cell(row=rev+2,column=10+v*7).value
        if float(int(score))==score: score=int(score)
        if score>4: score=4
        score = str(score)
        ...
        for sp in opin_sec:
            if sp != '':
                ...
                point+=1
        tb.rows[9].cells[0].text = opin_final.strip()
        ...
        if str(shtfull.cell(row=v+5,column=2).value)[0:4] != str(pln_num):
            ...
            rn = doc.paragraphs[11].add_run('\t'*6+'審查委員： '+rev_nam) 
            setFNT(rn)
        ...
        # if not os.path.exists(str(rev_nam)+'審查意見'): os.mkdir(str(rev_nam)+'審查意見')
        doc.save(str(pln_num)+str(rev_nam)+'_審查意見表.docx')
